...
% TODO: geh weniger auf den technischen outcome ein, sondern eher auf den untersuchungsgegenstand, das ergebnis und die abgrenzung zu deiner arbeit ...
...
The benefits of adoption of GitOps are presented well,
%however any possible drawbacks of an adoption are omitted from the analysis.
however there are not any possible problems discussed.
\citeauthor{whatIsGitOpsSalecha2023} (\citeyear{whatIsGitOpsSalecha2023})
discusses the definition of the term in the chapter "What Is GitOps?"
of the book "Practical GitOps"
...
publish a conference paper about implementing GitOps in IoT Edge Computing
...
The research focuses on the advantages compared to push-based deployment models.
\bigskip

\noindent
There has not been any scientific research conducted yet on the
specific topic of
...
discusses
the thought of
modeling different GitOps environments by using Git branches.
\citeauthor{codefreshStopUsingBranchesGitOpsEnvironments} (\citeyear{codefreshStopUsingBranchesGitOpsEnvironments})
...
about modeling GitOps environments and promoting releases between them.
...
\textit{The application version},
\textit{Kubernetes specific settings},
\textit{mostly static business settings},
and 
\textit{non-static business settings}.
While \textit{the application version} and \textit{non-static business settings} are promoted,
\textit{Kubernetes specific settings} and \textit{mostly static business settings} are generally not promoted between environments
...
has brought forward the tool
\citetitle{argocdAutopilotWebsite} \autocite{argocdAutopilotWebsite}
to help new GitOps adopters with
structuring their Git Repsitories,
and
...
%\autocite{codefreshStopUsingBranchesGitOpsEnvironments}
%
%% TODO: immer fließtext nach aufzählungen, tabellen, etc.
%
